...
fn create_branch_with_valid_name(
    ...
    // assert the refs path exists and it's a directory
    ...
    // Get the actual commit OID that HEAD points to (resolve symbolic references)
    let head_commit_oid = get_head_commit_sha(repository_dir.path())?;
    ...
    // assert the branch ref exists and its content is equal to the HEAD's commit OID
    ...
    let branch_ref_content = std::fs::read_to_string(&branch_ref_path)?
        .trim()
        .to_string();
    assert_eq!(branch_ref_content, head_commit_oid);
