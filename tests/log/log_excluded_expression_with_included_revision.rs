...
#[rstest]
fn log_excluded_expression_with_included_revision(
    ...
    // Test excluded expression: ^feature master
    ...
    assert!(
        ...
        "Expected 'Fourth commit' with excluded expression"
    );
    assert!(
        stdout.contains("    Third commit"),
        "Expected 'Third commit' with excluded expression"
    );
    ...
    assert!(
        ...
        "Should not include 'Feature commit 2' with excluded expression"
    );
