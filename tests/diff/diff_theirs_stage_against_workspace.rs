...
/// `bit diff -3` compares stage 3 (theirs) against the workspace file.
#[rstest]
fn diff_theirs_stage_against_workspace(
    ...
    let out = run_bit_command(dir.path(), &["diff", "-3"])
        .assert()
        .success()
        .get_output()
        .stdout
    ...
    // The patch should be present (stage-3 is the "before" state)
    assert!(
